...
- `menu_id` — ID меню, ингредиенты из которого загрузить в корзину (если не указано, ищется меню, в которое входит текущая дата)
...
Экспортирует продукты из корзины пользователя в виде списка.
Поддерживает параметр `?all=true`: все продукты из корзины
...
- `cart_items_ids` — список ID cart items, которые будут экспортированы (если список пустой, а параметр all не равен true, возвращает HTTP_400_BAD_REQUEST)

> Внимание: не удаляет экспортированные продукты из корзины
