...
**Неправильная настройка JWT — email в payload токена**

В `settings.py` в блоке `SIMPLE_JWT` была строчка `'USER_ID_FIELD': 'email'`. Токены при этом генерировались и авторизация формально работала, но такой подход создавал сразу две проблемы.

**Проблема 1 — чувствительные данные в токене.** JWT — это не зашифрованный, а лишь подписанный формат: payload можно прочитать без ключа на любом `jwt.io`. Email пользователя не должен попадать в токен — это персональные данные, которые при утечке токена (например, через логи или прокси) немедленно раскрываются.

**Проблема 2 — токен привязан к email.** Если пользователь меняет email, все его активные токены сразу становятся невалидными — потому что `user_id` в них содержит старый email, который больше не принадлежит этому аккаунту. Для пользователя это выглядит как внезапный разлогин без объяснений.

Строчку удалил. По умолчанию `USER_ID_FIELD = 'id'` — в токен попадает числовой PK пользователя: не несёт персональных данных и не меняется никогда. Авторизация по email настраивается через `User.USERNAME_FIELD = 'email'`, что у нас уже было — это две независимые вещи.
...
| P0 | `config/settings.py` | Удалил `USER_ID_FIELD: 'email'` из SIMPLE_JWT (email в токене — чувствительные данные; смена email инвалидировала все токены) |
