    ...
    def handle(self, *args, **options):
        ...
        for _ in range(user_count):
            ...
            while User.objects.filter(username=username).exists():
                username = fake.first_name()
            password = f'{random.randint(100, 999)}'
            portion_size = random.randint(1, 6)
            user_allergies = random.sample(allergies_list, k=random.randint(0, len(allergies_list)))
            user_diet_types = random.sample(diet_types_list, k=random.randint(0, len(diet_types_list)))
            ...
            user.diet_types.set(user_diet_types)
